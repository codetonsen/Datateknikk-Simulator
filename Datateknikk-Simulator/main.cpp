...
#include <string>
#include <utility>
class KeyControllerPeter : public threepp::KeyListener {
public:
    explicit KeyControllerPeter(threepp::Object3D& object_, std::unique_ptr<simple_socket::SimpleConnection> connection) : object(&object_), connection(std::move(connection)) {};
...
int main() {
    ...
    lidar.setOnNewFrameHandler([&](const LidarFrame& lidarFrame) {
        ...
        // Tilgang til roverFrame og lidarFrame
        frame = Frame(roverFrame, lidarFrame);
        auto lidarPoints = frame.lidarPoints;
        auto x = frame.x;
        auto y = frame.y;
        auto heading = frame.heading;
        std::pair<std::vector<std::pair<float, float>>, std::vector<float>> convertedFrame;
        for (auto& p : lidarPoints) {
            auto point = std::make_pair(p.x, p.y);
            convertedFrame.first.emplace_back(point);
        }
        convertedFrame.second = {x,y,heading};
        if (convertedFrame.first.size() > 25) {
            new_slam.enqueueFrame(convertedFrame);
        }
    ...
    scene->add(keyObject);
    KeyControllerPeter keyListener(keyObject, std::move(commandConnection));
