...
std::pair<std::vector<std::pair<float, float>>, std::vector<float>> Sphero::getSLAMframe() {
    ...
    if (lidarScans.size() >= lidarResolution_) {
        std::vector<float> posChange = {
            addNoise(this->position.x, 0.03f),
            addNoise(this->position.z, 0.03f),
            addNoise(this->rotation.y, 0.003f)
        };
        ...
        for (const auto& scan : lidarScans) {
            ...
            // Calculate x and y coordinates to world coordinates before adding them. This makes it easier for icp to handle changes
