...
std::vector<std::pair<std::vector<LidarScan>, std::vector<float>>> parseDataset(const std::string& filename, float offsetX = 0, float offsetY = 0, float offsetHeading = 0) {
  ...
  void visualizeWorldMap(std::vector<std::vector<std::pair<float, float>>> worldMap) {

    auto img = cv::Mat::zeros(img_size, img_size, CV_8UC3);

    // Color for the world map (cyan)
    cv::Scalar worldColor = cv::Scalar(255, 255, 0);

    for (const auto frame : worldMap) {

        float x_img = center + frame.first;
        float y_img = center + frame.second;
